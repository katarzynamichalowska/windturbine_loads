...
mse_loss_list = []
fft_mse_loss_list = []
fft_log_mse_loss_list = []
wasserstein_distance_loss_list = []
fft_wasserstein_distance_loss_list = []
cosine_similarity_list = []
generator_diversity_list = []
wt_cA_mse_loss_list = []
wt_cD1_mse_loss_list = []
wt_cD2_mse_loss_list = []
wt_cD3_mse_loss_list = []
...
for epoch in params.get('cp'):
    ...
    for i, (X_i, Y_i) in enumerate(dataloader):
        ...
        if params.get('compute_cosine_similarity'):
            #cosine_similarity += np.dot(Y_gen_i, Y_i) / (np.linalg.norm(Y_gen_i) * np.linalg.norm(Y_i))
            warnings.warn("Cosine similarity is not implemented.")
            pass
        ...
        if params.get('compute_wasserstein'):
            #wasserstein_distance_loss += wasserstein_distance(Y_i.flatten(), Y_gen_i.flatten())
